        ...
        <div className="animate-fade-in space-y-6 relative">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex items-center gap-3">
                    <button onClick={exportCSV} className="btn-ghost flex items-center gap-2 text-sm" title="Export CSV">
                        <Download size={16} />
                        <span className="hidden sm:inline">Export</span>
                    </button>
                    <button onClick={handleCreate} className="btn-primary flex items-center gap-2 text-sm shadow-lg shadow-brand-500/20">
                        <Plus size={16} />
                        <span>New Task</span>
                    </button>
                </div>
            ...
            <div className="glass-card rounded-2xl p-4 flex flex-col md:flex-row items-stretch md:items-center gap-3">
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-1 md:pb-0">
                    <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}
                        className="px-3 py-2.5 rounded-xl bg-surface-800/60 border border-white/5 text-gray-300 text-sm outline-none focus:ring-2 focus:ring-brand-500/30 cursor-pointer transition-all hover:bg-surface-800">
                    ...
                    <button onClick={() => setSortOrder(prev => prev === 'ASC' ? 'DESC' : 'ASC')}
                        className="p-2.5 rounded-xl bg-surface-800/60 border border-white/5 text-gray-400 hover:text-brand-400 hover:border-brand-500/20 transition-all active:scale-95"
                        title={`Sort ${sortOrder === 'ASC' ? 'Descending' : 'Ascending'}`}>
                        <ArrowUpDown size={16} />
                    </button>
                    {hasActiveFilters && (
                        <button onClick={clearFilters}
                            className="p-2.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 transition-all active:scale-95"
                            title="Clear filters">
                            <X size={16} />
                        </button>
                    )}
            ...
            <div className="space-y-3">
                {tasks?.map((task, index) => (
                    <div key={task.id} className="glass-card rounded-2xl overflow-hidden animate-slide-up group/card" style={{ animationDelay: `${index * 50}ms` }}>
                        <div className="p-4 flex flex-col sm:flex-row sm:items-center justify-between group gap-4 cursor-pointer" onClick={() => toggleTaskExpand(task.id)}>
                            <div className="flex items-start gap-4">
                                <button onClick={(e) => { e.stopPropagation(); updateStatusMutation.mutate({ id: task.id, status: task.status === 'DONE' ? 'TODO' : 'DONE' }); }}
                                    className={cn("text-gray-500 hover:text-brand-400 transition-all duration-200 mt-0.5 active:scale-90", task.status === 'DONE' && "text-emerald-400 hover:text-emerald-300")}>
                                    {task.status === 'DONE' ? <CheckCircle size={22} /> : <Circle size={22} />}
                                </button>
                                <div className="min-w-0">
                                    <h3 className={cn("font-semibold text-gray-100 text-base group-hover/card:text-white transition-colors", task.status === 'DONE' && "line-through text-gray-500 group-hover/card:text-gray-400")}>{task.title}</h3>
                                    {task.description && <p className="text-gray-500 text-sm mt-0.5 line-clamp-1">{task.description}</p>}
                                    <div className="flex flex-wrap items-center gap-2 mt-2">
                                        <span className={cn("px-2.5 py-0.5 rounded-lg text-xs font-medium",
                                            task.status === 'TODO' && "bg-gray-500/10 text-gray-400 border border-gray-500/10",
                                            task.status === 'IN_PROGRESS' && "bg-brand-500/10 text-brand-400 border border-brand-500/20",
                                        ...
                                        {task.category && (
                                            <span className="px-2.5 py-0.5 rounded-lg text-xs font-medium bg-purple-500/10 text-purple-400 border border-purple-500/20">{task.category}</span>
                                        )}
                                        <span className="text-xs text-gray-600 font-medium">{format(new Date(task.createdAt), 'MMM d, yyyy')}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-4 self-end sm:self-center">
                                {(timeTotals?.[task.id] || 0) > 0 && (
                                    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-violet-500/10 border border-violet-500/20">
                                        ...
                                        <span className="text-sm font-mono font-semibold text-violet-300 tabular-nums">{formatDuration(timeTotals?.[task.id] || 0)}</span>
                                    </div>
                                )}
                                <div className="text-gray-600 group-hover/card:text-gray-400 transition-colors">{expandedTaskId === task.id ? <ChevronUp size={18} /> : <ChevronDown size={18} />}</div>
                                <div className="flex items-center gap-1.5 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-300" onClick={e => e.stopPropagation()}>
                                    {activeTimer?.taskId === task.id ? (
                                        <button disabled className="p-2 rounded-xl bg-emerald-500/10 text-emerald-400 cursor-not-allowed"><CheckSquare size={16} /></button>
                                    ) : (
                                        <button onClick={() => startTimerMutation.mutate(task.id)}
                                            className="p-2 rounded-xl text-gray-500 hover:text-emerald-400 hover:bg-emerald-500/10 transition-all active:scale-90" title="Start Timer">
                                            <Play size={16} />
                                        </button>
                                    )}
                                    <button onClick={() => handleEdit(task)} className="p-2 rounded-xl text-gray-500 hover:text-amber-400 hover:bg-amber-500/10 transition-all active:scale-90"><Edit2 size={16} /></button>
                                    <button onClick={() => handleDelete(task.id)} className="p-2 rounded-xl text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-all active:scale-90"><Trash2 size={16} /></button>
                                </div>
                            </div>
                        </div>
...
function TaskTimeLogs({ taskId }: { taskId: string }) {
    ...
    return (
        <div className="border-t border-white/5 bg-surface-800/30 p-4 animate-fade-in">
            <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                    {todaySeconds > 0 && todaySeconds !== totalSeconds && (
                        <span className="text-xs font-mono font-semibold text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded-lg">
                            Today: {formatDuration(todaySeconds)}
                        </span>
                    )}
                    {totalSeconds > 0 && (
                        <span className="text-xs font-mono font-semibold text-violet-400 bg-violet-500/10 px-2 py-1 rounded-lg">
            ...
            {!isLoading && logs && logs.length === 0 && (
                <p className="text-xs text-gray-600 text-center py-3 italic">No time logged for this task yet</p>
            )}
                        ...
                        return (
                            <div key={dateKey}>
                                ...
                                <div className="space-y-1">
                                    {dateLogs.map((log) => (
                                        <div key={log.id} className="flex items-center justify-between py-2 px-3 rounded-lg bg-surface-800/40 text-xs border border-white/[0.02] hover:bg-surface-800/60 transition-colors">
                                            <div className="flex items-center gap-3 text-gray-400">
                                                <span className={cn(
                                                    "w-1.5 h-1.5 rounded-full",
                                                    log.endTime ? "bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.3)]" : "bg-amber-400 animate-pulse"
                                                )} />
                                                <span className="tabular-nums">{format(new Date(log.startTime), 'h:mm:ss a')}</span>
                                                {log.endTime && (
                                                    <>
                                                        <span className="text-gray-600">→</span>
                                                        <span className="tabular-nums">{format(new Date(log.endTime), 'h:mm:ss a')}</span>
                                                    </>
                                                )}
                                            </div>
                                            <span className="font-mono font-semibold text-gray-300 tabular-nums">
                                                {log.duration ? formatDuration(log.duration) : (
                                                    <span className="text-amber-400 text-[10px] uppercase tracking-wider animate-pulse font-bold">running...</span>
                                                )}
                                            </span>
                                        </div>
                                    ))}
...
function TaskModal({ isOpen, onClose, task, onToast }: { isOpen: boolean; onClose: () => void; task: Task | null; onToast: (m: string, t?: 'success' | 'error') => void }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-fade-in" onClick={onClose}>
            <div className="glass-card rounded-3xl shadow-glow-lg w-full max-w-md overflow-hidden animate-scale-in border-white/10" onClick={(e) => e.stopPropagation()}>
                <div className="px-6 py-5 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                    <h2 className="text-xl font-bold text-white tracking-tight">{task ? 'Edit Task' : 'New Task'}</h2>
                    <button onClick={onClose} className="p-2 rounded-xl text-gray-500 hover:text-gray-300 hover:bg-white/5 transition-all outline-none"><X size={20} /></button>
                </div>
                <form onSubmit={handleSubmit((data) => mutation.mutate(data))} className="p-6 space-y-4">
                    <div>
                        <label className="block text-sm font-semibold text-gray-300 mb-2">Title <span className="text-red-400">*</span></label>
                        ...
                        {errors.title && <p className="text-[11px] text-red-400 mt-1.5 ml-1 font-medium">{errors.title.message}</p>}
                    </div>
                    <div>
...
function StopwatchBanner({ startTime, taskTitle, onStop }: { startTime: string; taskTitle: string; onStop: () => void }) {
    ...
    return (
        <div className="rounded-3xl overflow-hidden animate-scale-in shadow-2xl shadow-emerald-500/10 border border-emerald-500/20">
            <div className="h-1.5 bg-gradient-to-r from-emerald-400 via-emerald-500 to-teal-500 animate-pulse" />
            <div className="bg-gradient-to-r from-emerald-500/[0.08] via-emerald-500/[0.04] to-teal-500/[0.08] backdrop-blur-xl p-6">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        <div className="relative">
                            ...
                            <div className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-red-500 animate-pulse border-[3px] border-surface-900" />
                        </div>
                        <div>
                            <div className="flex items-baseline gap-0.5 font-mono">
                                <span className="text-4xl font-black text-emerald-300 tabular-nums tracking-tighter">{time.hours}</span>
                                <span className="text-3xl font-black text-emerald-500/50 mx-0.5">:</span>
                                <span className="text-4xl font-black text-emerald-300 tabular-nums tracking-tighter">{time.minutes}</span>
                                <span className="text-3xl font-black text-emerald-500/50 mx-0.5">:</span>
                                <span className="text-4xl font-black text-emerald-300 tabular-nums tracking-tighter">{time.seconds}</span>
                            </div>
                            <p className="text-sm font-medium text-emerald-400/80 mt-1.5 flex items-center gap-2">
                                <span className="w-2 h-2 rounded-full bg-emerald-400 inline-block animate-pulse" />
                                <span className="opacity-80">Currently Tracking:</span>
                                <span className="font-bold text-emerald-400">{taskTitle}</span>
                            </p>
                        </div>
                    </div>
                    <button onClick={onStop}
                        className="px-6 py-3 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 rounded-2xl text-sm font-bold transition-all active:scale-95 flex items-center gap-3 hover:shadow-xl hover:shadow-red-500/10 group">
                        <div className="w-3.5 h-3.5 rounded-sm bg-red-400 group-hover:scale-110 transition-transform" />
                        Stop Timer
                    </button>
                </div>
            </div>
